...
@section('title', 'Item Menus')
@section('css')
    <link rel="stylesheet" type="text/css" href="https://cdnjs.cloudflare.com/ajax/libs/Dropify/0.2.2/css/dropify.min.css">
@endsection
@section('content')
    <section class="invoice-list-wrapper">
        <h3 class="mb-3">Items Menus List</h3>
        <div class="card">
                        ...
                        <thead>
                            <tr role="row">
                                ...
                                <th class="cell-fit sorting_disabled" rowspan="1" colspan="1" style="width: 80px;"
                                    aria-label="Actions">Item Name</th>
                                <th class="cell-fit sorting_disabled" rowspan="1" colspan="1" style="width: 80px;"
                                    aria-label="Actions">Name</th>
                                <th class="cell-fit sorting_disabled" rowspan="1" colspan="1" style="width: 80px;"
                                    aria-label="Actions">Price</th>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="javascript:;" class="edit-category btn btn-sm btn-info  waves-effect waves-light" title="Update"
                                                data-id="{{ $item->id }}" data-price = "{{ $item->price }}"    data-name="{{ $item->name }}" data-item = "{{ $item->item_id }}" 
                                                >
                ...
                <div class="modal-body pb-5 px-sm-5 pt-50">
                    ...
                    <form id="addForm" class="row gy-1 pt-75" onsubmit="return false">
                        ...
                        <div class="col-12 col-md-12">
                            ...
                            <select class="form-control item select-child" name="item_id" id="item_id" required>
                            ...
                            <input type="text" id="menu_name" name="menu_name"
                                class="form-control @error('menu_name') is-invalid @enderror" placeholder="Menu Name"
                                data-msg="Please enter Item menu name" />
                            @error('menu_name')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            @enderror
                            <label class="form-label" for="code">Price($)</label>
...
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '#btn_save', function() {
            ...
            if (validate) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('item-menu.store') }}", // your php file name
                    data: form_data,
                    dataType: "json",
                    processData: false,
                    contentType: false,
  ...
  console.log($(this).data('price'));
            $('.text-title').text('Edit Item Menu');
            // Dropify Edit image code

            $('#id').val($(this).data('id'));
            $('#item_id').val($(this).data('item'));
        ...
        $(document).on('click', '.delete-permission', function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.ajax({
                        type: "POST",
                        url: "{{ route('item-menu.delete') }}", // your php file name
                        data: form_data,
                        dataType: "json",
                        processData: false,
                        contentType: false,
