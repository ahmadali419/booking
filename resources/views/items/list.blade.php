                        ...
                        <thead>
                            <tr role="row">
                                ...
                                <th class="cell-fit sorting_disabled" rowspan="1" colspan="1" style="width: 80px;"
                                    aria-label="Actions">Service</th>
                                <th class="cell-fit sorting_disabled" rowspan="1" colspan="1" style="width: 80px;"
                                    aria-label="Actions">Item Name</th>
                                <th class="cell-fit sorting_disabled" rowspan="1" colspan="1" style="width: 80px;"
                                    aria-label="Actions">Image</th>
                            ...
                            @if (!$lists->isEmpty())
                                @foreach ($lists as $item)
                                    <tr>
                                        ...
                                        <td>{{ $item->name }}</td>
                                        <td><img src="{{ asset($item->image) }}" width="75"></td>
                                        <td>
                                            <a href="javascript:;" class="edit-category btn btn-sm btn-info  waves-effect waves-light" title="Update"
                                                data-id="{{ $item->id }}"   data-thumbnail="{{ asset($item->image) }}" data-image="{{ $item->image }}" data-name="{{ $item->name }}" data-service = "{{ $item->service_id }}" 
                                                >
                ...
                <div class="modal-body pb-5 px-sm-5 pt-50">
                    ...
                    <form id="addForm" class="row gy-1 pt-75" onsubmit="return false">
                        ...
                        <div class="col-12 col-md-12">
                            ...
                            <select class="form-control service select-child" name="service_id" id="service_id" required>
                                <option value="">Select..</option>
                                @foreach($services as $item)
                                <option  value="{{ $item->id }}">{{ $item->name }}</option>
                                @endforeach
                            </select> 
                            <label class="form-label" for="code">Item Name</label>
                            <input type="text" id="item_name" name="item_name"
                                class="form-control @error('item_name') is-invalid @enderror" placeholder="Item Name"
                        ...
                        <input type="hidden" name="old_image" class="old-image" value="">
                        <div class="col-12 col-md-12">
                            <input type="file" name="image" class="getrecomendationimg @error('image') is-invalid @enderror dropify"
                                id="image" >
                            @error('image')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
